...
def mapjson (injs):
	def meetscondition (record):
		keys = ["year", "id", "abstract", "references", "n_citation", "authors"]
		return np.all ([key in record for key in keys]) and len (record["abstract"]) > 0
	...
	if meetscondition (injs):
		...
		outjs["id"] = injs["id"]
		outjs["abstract"] = injs["abstract"]
		outjs["outcites"] = injs["references"]
		outjs["nincites"] = injs["n_citation"]
