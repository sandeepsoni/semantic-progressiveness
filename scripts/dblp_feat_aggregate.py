...
def scoredist (filename):
	...
	with open (filename) as fin:
		for i, line in enumerate (fin):
			js = ujson.loads (line)
			scores.extend([item["score"] for item in js])

	progressives = np.array (scores)
	return scores

def readSemFeats (filename, percentiles=[99, 95, 90, 75, 50, 25], check_every=1000000, verbose=False):
	records = list ()
	...
	with open (filename) as fin:
		for i, line in enumerate (fin):
			js = ujson.loads (line)
			items = [(item["score"], item["word"]) for item in js]
			...
			records.append ([nprog, nreg, max_prog, max_prog_word, max_reg, max_reg_word] + npercentiles + [str(len(items))])
...
def main (srcdir,
		  ...
		  docid="dblp.id", 
		  indeg="dblp.nincites",
		  outdeg="dblp.noutcites",
		  year="dblp.year",
		  authors="dblp.nauthors",
		  nuniqs="dblp.nuniqs",
		  ntokens="dblp.ntokens",
		  lang="dblp.langid",
		  bows="dblp.bowscores",
		  sems="dblp.semscores"):
	...
	df_year = pd.read_csv (os.path.join (srcdir, year), sep=",", header=None, names=["docid", "year"])
	df_authors = pd.read_csv (os.path.join (srcdir, authors), sep=",", header=None, names=["docid", "nauthors"])
	...
	df_lang = pd.read_csv (os.path.join (srcdir, lang), sep=",", header=None, names=["lang"])
