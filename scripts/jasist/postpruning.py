...
def main (args):
	...
	early = readCaseStats (args.early_file)
	later = readCaseStats (args.later_file)

	vocab = readWordScores (args.change_list_file)
	top_words = list (map(lambda x:x[0], sorted (vocab.items(), key=lambda x:x[1], reverse=True)))
	names = readNames (args.best_tag_file)
	...
	with open (args.target_file, "w") as fout:
		for word in keep[0:args.top]:
			fout.write ("{}\n".format (word))
